...
class Proxy:
    def __init__(self, gcs_conf=default_gcs_conf, ac_conf=None, verbose=False):
        ...
        self.ivy = pprzlink.ivy.IvyMessagesInterface("Pprzlink_proxy UDPLink", True, False, pprzlink.ivy.IVY_BUS)
    ...
    def proccess_mav_msg(self, sender, address, msg, length, receiver_id=None, component_id=None):
        if receiver_id is None:
            return
        elif str(msg).find("AUTOPILOT_VERSION") > 0: # TODO: Solve incompatibility with Python3 
            return
    ...
    def proccess_uplink_msg(self, receiver_id, msg):
        receiver_id = int(receiver_id)
        for (ac_id, ac_addr, m) in self.acs:
            if receiver_id == ac_id:
                # send ivy datalink message to correct MAV
                if self.verbose:
                    print("sending to %i (%i): %s" % (receiver_id, ac_id, msg))
                m.send(msg, 0, ac_addr, ac_id)
