...
// TODO: Debugging section, delete it when the driver is finished
uint8_t debug_1 = 2;
uint8_t debug_2 = 2;
float debug_3 = 2;
uint16_t debug_4 = 2;
uint16_t debug_5 = 2;
struct invensense3_t *debug_inv;
uint8_t *debug_data;
uint8_t debug_data_len = 14;
uint8_t debug_status = 100;

// TODO: Debugging section, delete it when the driver is finished
static void send_debug(struct transport_tx *trans, struct link_device *dev)
{ 
  uint16_t debug_gyro_samplerate = debug_inv->gyro_odr;
  uint16_t debug_accel_samplerate = debug_inv->accel_odr;
  pprz_msg_send_MY_DEBUG(trans, dev, AC_ID, &debug_1, &debug_2, &debug_3, &debug_4, &debug_5,
                                            &debug_gyro_samplerate, &debug_accel_samplerate,
                                            4, debug_inv->gyro_aaf_regs,
                                            4, debug_inv->accel_aaf_regs,
                                            &debug_status,
                                            42, debug_data);
}
...
void invensense3_init(struct invensense3_t *inv) {
  ...
  // TODO: Debugging section, delete it when the driver is finished
  debug_inv = inv;
  register_periodic_telemetry(DefaultPeriodic, PPRZ_MSG_ID_MY_DEBUG, send_debug);
}
    ...
    switch(inv->status) {
      ...
      case INVENSENSE3_CONFIG:
        ...
        if(invensense3_config(inv)) {
          inv->status = INVENSENSE3_CHECKCONFIG;
        }
        break;

      case INVENSENSE3_CHECKCONFIG: // TODO: Debugging section, delete it when the driver is finished (OR build a config checker)
        //invensense3_register_read(inv, INV3REG_FIFO_CONFIG1, 1);
        //invensense3_register_read(inv, INV3REG_FIFO_CONFIG2, 2);
        //invensense3_register_read(inv, INV3REG_INTF_CONFIG0, 1);
        //invensense3_register_read(inv, INV3REG_PWR_MGMT0, 1);
        //invensense3_register_read(inv, INV3REG_GYRO_CONFIG0, 1);
        //invensense3_register_read(inv, INV3REG_ACCEL_CONFIG0, 1);
        invensense3_register_read(inv, INV3REG_INT_STATUS, 1);
        //invensense3_register_read(inv, INV3REG_INT_SOURCE0, 1);
        break;
  ...
  if((inv->bus == INVENSENSE3_SPI && inv->spi.trans.status == SPITransSuccess) || 
    ...
    switch(inv->status) {
      ...
      case INVENSENSE3_CONFIG:
        ...
        if(invensense3_config(inv)) {
          inv->status = INVENSENSE3_CHECKCONFIG; // TODO: Debugging section, change to running when the driver is finished (OR build a config checker)
        }
        break;

      case INVENSENSE3_CHECKCONFIG:
        // debug_4 += 1;
        // debug_1 = rx_buffer[1];
        debug_2 = inv->parser;
        // debug_status += rx_buffer[1];
      ...
      case INVENSENSE3_RUNNING:
        ...
        switch (inv->parser) {
          ...
          case INVENSENSE3_PARSER_FIFO: {
            ...
            debug_3 += n_samples;
            // debug_2 = rx_buffer[1];
            RMAT_COPY(debug_data, rx_buffer);
            // debug_5 += 1;
 ...
 * 
 * @param inv The invensense v3 instance
 * @param data The data from all registers
 */
static void invensense3_parse_reg_data(struct invensense3_t *inv, volatile uint8_t *data) {
